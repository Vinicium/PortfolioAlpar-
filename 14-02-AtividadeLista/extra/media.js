...
function numeroAleatorio(min, max) {
  //dava pra usar pra fazer nomes aleatorios tbm
...
function numeroEquipe(nomes) {
  let equipe = nomes.map((nome) => {
    ...
    let pontuacoes = Array.from({ length: numTarefas }, () =>
      numeroAleatorio(0, 10)
    ); //gera a nota das tarefas
...
function analisarDesempenho(equipe) {
  ...
  resultados.forEach(function (membro) {
    //foreach roda por todos os itens do array
    console.log(membro.nome + ": Média " + membro.media);
  });

  console.log(
    "Melhor desempenho: " + resultados[0].nome + " " + resultados[0].media
  );
  console.log(
    "Pior desempenho: " +
      resultados[resultados.length - 1].nome +
      " " +
      resultados[resultados.length - 1].media
  );
}
